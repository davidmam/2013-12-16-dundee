...
This morning, after installing git, we asked everyone to run
```
git clone http://github.com/chryswoods/2013-11-14-exeter 
```
This created a copy of the software carpentry repository materials on
each of your hard drives yesterday morning.   If you did this earlier,
you don't need to to it again.

*But*, over lunch, the instructors changed the content on github, so now
...
We won't be doing the next two steps in the session but you can note them for later.
...
create a file, then we'll learn the **git add** command. Adding files is also referred to 
as *staging* them. Files which are *staged* are those for which we have told git we wish 
the current changes to be tracked and eventually committed. We can change or create any 
file we wish, but unless it is staged (added), git will ignore it.
...
The changes your make in this local copy aren't preserved
automatically. Until you commit them, the changes you make are temporary
changes, even after you save the file. When you change anything, your set of files becomes different
...
The null result would mean that you're working files (the ones you are working on right now) 
are up to date with the current version of the repository on your hard drive.

## git commit : Saving a Snapshot

In order to save a snapshot of the current state (revision or version) of the
repository, we use the **git commit** command. This command is always associated
...
Let's recall the behaviour of the diff command on the command line.
